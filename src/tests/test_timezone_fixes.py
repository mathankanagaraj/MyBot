...
import pytz
from core.angelone.utils import get_ist_now, utc_to_ist, IST
...
def test_15m_boundary_detection():
    ...
    for hour, minute, expected in test_cases:
        # Create IST datetime
        test_time = datetime.now(IST).replace(
            hour=hour, minute=minute, second=30, microsecond=0
        )
        ...
        print(
            f"{status} {test_time.strftime('%H:%M:%S')} -> {result} (expected {expected}), sleep {sleep_seconds}s"
        )
...
def test_bot_startup_scenarios():
    ...
    for description, hour, minute, expected_boundary in scenarios:
        start_time = datetime.now(IST).replace(
...
def test_data_fetch_duration():
    ...
    if abs(duration_minutes - 15.0) < 0.1:
        ...
        print(
            f"✗ Duration is incorrect (expected 15 minutes, got {duration_minutes:.2f})"
        )
